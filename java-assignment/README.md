...
- Applied **SOLID principles** throughout — single responsibility per class, dependency inversion via port interfaces (e.g. `WarehouseStore`, `LocationResolver`)
...
- **Bug fix**: Fixed `StoreResource.patch()` checking wrong fields for partial update (see [QUESTIONS.md](QUESTIONS.md) for details)
- **Test coverage**: 76 tests across unit, integration (`@QuarkusTest`), and smoke (`@QuarkusIntegrationTest`) layers; JaCoCo integrated via `quarkus-jacoco` — run `./mvnw test` and view the report at `target/jacoco-report/index.html`
